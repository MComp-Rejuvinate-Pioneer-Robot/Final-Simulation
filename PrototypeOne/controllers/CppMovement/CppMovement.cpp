...
void positionSensorSetup(int timestep) {
  vector<string> wheelSensorNames = {"back left wheel sensor", "back right wheel sensor", "front left wheel sensor", "front right wheel sensor"};
...
void PIDPositionSensor() {
  PIDController leftWallPID = PIDController(0.1, MAX_SPEED, -MAX_SPEED, 2, 0.5, 0.5);
  PIDController rightWallPID = PIDController(0.1, MAX_SPEED, -MAX_SPEED, 2, 0.5, 0.5);
  PIDController leftPosPID = PIDController(0.1, MAX_SPEED, -MAX_SPEED, 2, 0.5, 0.5);
  PIDController rightPosPID = PIDController(0.1, MAX_SPEED, -MAX_SPEED, 2, 0.5, 0.5);

  double leftInput = sensorVal(0);
  double rightInput = sensorVal(7);
  double frontInput = sensorVal(4);
  
  double leftPosInput = getPosition(2);
  double rightPosInput = getPosition(3);
  ...
  double output = leftWallPID.calculate(setpoint, leftInput);
  leftSetpoint = leftSpeed + output;
  rightSetpoint = rightSpeed - output;
  
  moveAllMotors(leftSetpoint, rightSetpoint);
}
